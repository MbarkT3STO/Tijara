...
/**
 * Generic print preview — accepts pre-built HTML string.
 * Shows the same overlay with zoom, print and PDF export.
 * Used by sales orders, purchase orders, and any other non-invoice document.
 */
export function openHtmlPrintPreview(html: string, title: string, filename: string): void {
  // ── Overlay ───────────────────────────────────────────────────────────────
  const overlay = document.createElement('div');
  overlay.id = '__print-preview-overlay';
  overlay.style.cssText = `
  ...
  const left = document.createElement('div');
  left.style.cssText = 'display:flex;align-items:center;gap:var(--space-3);';
  left.innerHTML = `<span style="font-weight:600;font-size:var(--font-size-sm);color:var(--color-text-primary);">${i18n.t('common.print')} — ${title}</span>`;
  ...
  const pageWrapper = document.createElement('div');
  pageWrapper.style.cssText = `
    width: 794px; transform-origin: top center;
    transition: transform 150ms ease;
    box-shadow: 0 4px 32px rgba(0,0,0,0.5);
  `;

  const iframe = document.createElement('iframe');
  iframe.style.cssText = 'width:100%;border:none;display:block;background:#fff;min-height:1123px;';
  ...
  iframe.onload = () => {
    const body = iframe.contentDocument?.body;
    if (body) iframe.style.height = Math.max(1123, body.scrollHeight + 48) + 'px';
  };
  ...
  const applyZoom = () => {
    ...
    zoomLabel.textContent = `${zoom}%`;
    pageWrapper.style.marginBottom = zoom < 100
      ? `${-(iframe.offsetHeight * (1 - zoom / 100))}px` : '0';
  };
  zoomOut.addEventListener('click', () => { zoom = Math.max(50, zoom - 10); applyZoom(); });
  zoomIn.addEventListener('click', () => { zoom = Math.min(150, zoom + 10); applyZoom(); });
  ...
  pdfBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      if (electron?.exportReportPDF) {
        await electron.exportReportPDF(html, filename);
      } else {
        iframe.contentWindow?.print();
      }
    } finally {
